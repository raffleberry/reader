...
- Demo build: `VITE_DEMO=1` (`just demo` / `bun run build:demo` → `dist-demo`, relative `--base ./`) flips `demo.ts:IS_DEMO`: `api/voice.ts` throws a download-hint error instead of fetching, `player.start` surfaces it as a phase error, `Reader.vue` shows a demo banner (+ error link, no read-from-here), `SettingsPanel.vue` skips server fetch and disables voice-save/cache controls. Hash history (`router.ts`) so refresh works on static hosts; `public/sw.js` registered from `main.ts` makes the shell offline-first (books are already in IDB); `App.vue` shows an offline pill. `public/books/` + `library/seed.ts` pre-load 3 bundled epubs into IDB on first demo run (stable ids, one-shot flag, deletable like uploads). Everything else (reading, TOC, bookmarks, themes) is untouched.
